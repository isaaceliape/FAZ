---
name: fase:contexto
description: Exibe, limpa ou resume o contexto da última sessão de trabalho armazenado em .fase-ai-local/CONTEXTO.md
...
<objective>
Gerencie o contexto persistente de sessão do FASE armazenado em `.fase-ai-local/CONTEXTO.md`.
...
```bash
if [ -f .fase-ai-local/CONTEXTO.md ]; then
  cat .fase-ai-local/CONTEXTO.md
else
  echo "Nenhum contexto de sessão encontrado. Execute /fase-executar-fase para iniciar."
fi
```

3. **Com `--limpar` — apagar contexto:**
```bash
rm -f .fase-ai-local/CONTEXTO.md
echo "Contexto de sessão removido. A próxima sessão começará do zero."
```

4. **Com `--resumo` — gerar resumo legível:**
Leia `.fase-ai-local/CONTEXTO.md` e produza um parágrafo resumindo:
