...
1. Se o pacote fase-ai está instalado globalmente
...
### 1. Verificar Instalação Global do Pacote
```bash
npm list -g fase-ai 2>/dev/null || echo "NAO_INSTALADO_GLOBAL"
```
...
**OpenCode:**
```bash
test -d ~/.config/opencode && echo "EXISTS" || echo "NAO_EXISTE"
test -f ~/.config/opencode/opencode.json && echo "EXISTS" || echo "NAO_EXISTE"
```
...
### Se pacote não instalado globalmente:
```bash
