...
function validPlanContent({ etapa = 1, dependsOn = '[]', autonomous = 'true', extraTasks = '' } = {}) {
  return [
    '---',
    'phase: 01-test',
    'plan: 01',
...
describe('validate consistency command', () => {
  ...
  test('passes for consistent project', () => {
    ...
    fs.mkdirSync(path.join(tmpDir, '.fase-ai-local', 'etapas', '03-c'), { recursive: true });
  ...
  test('warns about phase on disk but not in roadmap', () => {
    ...
    fs.mkdirSync(path.join(tmpDir, '.fase-ai-local', 'etapas', '01-a'), { recursive: true });
    fs.mkdirSync(path.join(tmpDir, '.fase-ai-local', 'etapas', '02-orphan'), { recursive: true });
...
describe('verify plan-structure command', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempProject();
    fs.mkdirSync(path.join(tmpDir, '.fase-ai-local', 'etapas', '01-test'), { recursive: true });
  });
  ...
  test('reports missing required frontmatter fields', () => {
    const planPath = path.join(tmpDir, '.fase-ai-local', 'etapas', '01-test', '01-01-PLAN.md');
    fs.writeFileSync(planPath, '# No frontmatter here\n\nJust a plan without YAML.\n');

    const result = runGsdTools('verify plan-structure .fase-ai-local/phases/01-test/01-01-PLAN.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('warns about etapa > 1 with empty depends_on', () => {
    ...
    const output = JSON.parse(result.output);
    assert.ok(
      output.warnings.some(w => w.includes('Etapa > 1 but depends_on is empty')),
      `Expected "Etapa > 1 but depends_on is empty" in warnings: ${JSON.stringify(output.warnings)}`
    );
  });
  ...
  test('returns error for nonexistent file', () => {
    const result = runGsdTools('verify plan-structure .fase-ai-local/phases/01-test/nonexistent.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('verify summary command', () => {
  ...
  test('returns not found for nonexistent summary', () => {
    const result = runGsdTools('verify-summary .fase-ai-local/phases/01-test/nonexistent.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('passes for valid summary with real files and commits', () => {
    ...
    const summaryPath = path.join(tmpDir, '.fase-ai-local', 'etapas', '01-test', '01-01-SUMMARY.md');
    ...
    const result = runGsdTools('verify-summary .fase-ai-local/phases/01-test/01-01-SUMMARY.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('respects checkFileCount parameter', () => {
    ...
    const result = runGsdTools('verify-summary .fase-ai-local/phases/01-test/01-01-SUMMARY.md --check-count 1', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('verify references command', () => {
  ...
  test('reports valid when all referenced files exist', () => {
    ...
    const filePath = path.join(tmpDir, '.fase-ai-local', 'etapas', '01-test', 'doc.md');
    fs.writeFileSync(filePath, '@src/app.js\n');

    const result = runGsdTools('verify references .fase-ai-local/phases/01-test/doc.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('returns error for nonexistent file', () => {
    const result = runGsdTools('verify references .fase-ai-local/phases/01-test/nonexistent.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('verify artifacts command', () => {
  ...
  test('passes when all artifacts exist and match criteria', () => {
    ...
    const result = runGsdTools('verify artifacts .fase-ai-local/phases/01-test/01-01-PLAN.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('verify key-links command', () => {
  ...
  test('verifies link when pattern found in source', () => {
    ...
    const result = runGsdTools('verify key-links .fase-ai-local/phases/01-test/01-01-PLAN.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
