...
describe('verify plan-structure command', () => {
  ...
  test('returns error for nonexistent file', () => {
    const result = runGsdTools('verify plan-structure .planejamento/phases/01-test/nonexistent.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('verify summary command', () => {
  ...
  test('returns not found for nonexistent summary', () => {
    const result = runGsdTools('verify-summary .planejamento/phases/01-test/nonexistent.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('passes for valid summary with real files and commits', () => {
    ...
    const result = runGsdTools('verify-summary .planejamento/phases/01-test/01-01-SUMMARY.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
  ...
  test('respects checkFileCount parameter', () => {
    ...
    const result = runGsdTools('verify-summary .planejamento/phases/01-test/01-01-SUMMARY.md --check-count 1', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
...
describe('verify references command', () => {
  ...
  test('returns error for nonexistent file', () => {
    const result = runGsdTools('verify references .planejamento/phases/01-test/nonexistent.md', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);
