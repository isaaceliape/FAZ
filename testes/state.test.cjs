...
**Resume File:** .fase-ai-local/phases/03-api/03-02-PLAN.md
`
    );
    ...
    assert.strictEqual(output.session.resume_file, '.fase-ai-local/phases/03-api/03-02-PLAN.md', 'resume file extracted');
  });
...
`
    ...
    const content = fs.readFileSync(path.join(tmpDir, '.fase-ai-local', 'STATE.md'), 'utf-8');
...
const { stateExtractField, stateReplaceField } = require('../bin/lib/state.cjs');
...
describe('cmdStateGet (state get)', () => {
  ...
  test('returns full content when no section specified', () => {
    ...
    fs.writeFileSync(path.join(tmpDir, '.fase-ai-local', 'STATE.md'), stateContent);
...
describe('cmdStatePatch and cmdStateUpdate (state patch, state update)', () => {
  ...
  test('state patch updates multiple fields at once', () => {
    fs.writeFileSync(path.join(tmpDir, '.fase-ai-local', 'STATE.md'), stateMd);

    const result = runGsdTools('state patch --Status Complete --"Current Phase" 04', tmpDir);
    assert.ok(result.success, `Command failed: ${result.error}`);

    const updated = fs.readFileSync(path.join(tmpDir, '.fase-ai-local', 'STATE.md'), 'utf-8');
...
describe('cmdStateAdvancePlan (state advance-plan)', () => {
  ...
  test('advances plan counter when not on last plan', () => {
    fs.writeFileSync(path.join(tmpDir, '.fase-ai-local', 'STATE.md'), advanceFixture);
  ...
  test('marks phase complete on last plan', () => {
    ...
    fs.writeFileSync(path.join(tmpDir, '.fase-ai-local', 'STATE.md'), lastPlanFixture);
...
describe('cmdStateRecordMetric (state record-metric)', () => {
  ...
  test('appends metric row to existing table', () => {
    fs.writeFileSync(path.join(tmpDir, '.fase-ai-local', 'STATE.md'), metricsFixture);
  ...
  test('replaces None yet placeholder with first metric', () => {
    ...
    fs.writeFileSync(path.join(tmpDir, '.fase-ai-local', 'STATE.md'), noneYetFixture);
...
describe('cmdStateUpdateProgress (state update-progress)', () => {
  ...
  test('calculates progress from plan/summary counts', () => {
    ...
    const phase01Dir = path.join(tmpDir, '.fase-ai-local', 'etapas', '01');
    ...
    const phase02Dir = path.join(tmpDir, '.fase-ai-local', 'etapas', '02');
...
describe('cmdStateResolveBlocker (state resolve-blocker)', () => {
  ...
  test('removes matching blocker line (case-insensitive substring match)', () => {
    fs.writeFileSync(path.join(tmpDir, '.fase-ai-local', 'STATE.md'), blockerFixture);
  ...
  test('adds None placeholder when last blocker resolved', () => {
    ...
    fs.writeFileSync(path.join(tmpDir, '.fase-ai-local', 'STATE.md'), singleBlockerFixture);
...
describe('cmdStateRecordSession (state record-session)', () => {
  ...
  test('updates session fields with stopped-at and resume-file', () => {
    fs.writeFileSync(path.join(tmpDir, '.fase-ai-local', 'STATE.md'), sessionFixture);

    const result = runGsdTools(
      'state record-session --stopped-at "Etapa 3, Plan 2" --resume-file ".fase-ai-local/phases/03/03-02-PLAN.md"',
      tmpDir
    );
    assert.ok(result.success, `Command failed: ${result.error}`);
    ...
    assert.ok(updated.includes('.fase-ai-local/phases/03/03-02-PLAN.md'), 'Resume file should be updated');
