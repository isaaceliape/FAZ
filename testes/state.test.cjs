...
`
    ...
    const content = fs.readFileSync(path.join(tmpDir, '.planejamento', 'STATE.md'), 'utf-8');
...
describe('cmdStateGet (state get)', () => {
  ...
  test('returns full content when no section specified', () => {
    ...
    fs.writeFileSync(path.join(tmpDir, '.planejamento', 'STATE.md'), stateContent);
...
describe('cmdStateAdvancePlan (state advance-plan)', () => {
  ...
  test('advances plan counter when not on last plan', () => {
    fs.writeFileSync(path.join(tmpDir, '.planejamento', 'STATE.md'), advanceFixture);
  ...
  test('marks phase complete on last plan', () => {
    ...
    fs.writeFileSync(path.join(tmpDir, '.planejamento', 'STATE.md'), lastPlanFixture);
...
describe('cmdStateRecordMetric (state record-metric)', () => {
  ...
  test('appends metric row to existing table', () => {
    fs.writeFileSync(path.join(tmpDir, '.planejamento', 'STATE.md'), metricsFixture);
  ...
  test('replaces None yet placeholder with first metric', () => {
    ...
    fs.writeFileSync(path.join(tmpDir, '.planejamento', 'STATE.md'), noneYetFixture);
...
describe('cmdStateResolveBlocker (state resolve-blocker)', () => {
  ...
  test('removes matching blocker line (case-insensitive substring match)', () => {
    fs.writeFileSync(path.join(tmpDir, '.planejamento', 'STATE.md'), blockerFixture);
  ...
  test('adds None placeholder when last blocker resolved', () => {
    ...
    fs.writeFileSync(path.join(tmpDir, '.planejamento', 'STATE.md'), singleBlockerFixture);
