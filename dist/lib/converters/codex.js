 ...
 */
import { extractFrontmatterAndBody, extractFrontmatterField, toSingleLine, yamlQuote, applyPathReplacements, convertSlashCommandsToSkillMentions, } from '../conversion-utils.js';
/**
 ...
 */
function convertCodexAgentFrontmatter(content) {
 ...
 */
function convertCodexCommandSkill(content, skillName) {
    const converted = convertClaudeToCodexMarkdown(content);
    const { frontmatter, body } = extractFrontmatterAndBody(converted);
 ...
 */
function flattenToSkillName(filename) {
    // Remove .md extension
