...
export function cmdRoadmapGetPhase(cwd, etapaNum, raw) {
    ...
    try {
        ...
        if (!headerMatch) {
            const checklistPattern = new RegExp(`-\\s*\\[[ x]\\]\\s*\\*\\*(?:Phase|Etapa)\\s+${escapedEtapa}:\\s*([^*]+)\\*\\*`, 'i');
...
export function cmdRoadmapAnalyze(cwd, raw) {
    const roadmapPath = path.join(cwd, '.fase-ai', 'ROADMAP.md');
    if (!fs.existsSync(roadmapPath)) {
    ...
    const nextEtapa = phases.find((p) => p.disk_status === 'empty' ||
        p.disk_status === 'no_directory' ||
        p.disk_status === 'discussed' ||
        p.disk_status === 'researched') ?? null;
...
export function cmdRoadmapUpdatePlanProgress(cwd, etapaNum, raw) {
    if (!etapaNum) {
    ...
    const planCountPattern = new RegExp(`(#{2,4}\\s*(?:Phase|Etapa)\\s+${phaseEscaped}[\\s\\S]*?\\*\\*Plans:\\*\\*\\s*)[^\\n]+`, 'i');
