...
import { escapeRegex, normalizeEtapaNome, compareEtapaNum, findEtapaInternal, getArchivedEtapasDirs, generateSlugInternal, getMilestoneEtapaFilter, toPosixPath, output, error, } from './core.js';
import { extractFrontmatter } from './frontmatter.js';
import { writeStateMd } from './state.js';
export function cmdEtapasList(cwd, options, raw) {
...
export function cmdEtapaNextDecimal(cwd, basePhase, raw) {
    const etapasDir = path.join(cwd, '.fase-ai', 'etapas');
    const normalized = normalizeEtapaNome(basePhase);
    if (!fs.existsSync(etapasDir)) {
        output({ found: false, base_phase: normalized, next: `${normalized}.1`, existing: [] }, raw, `${normalized}.1`);
        return;
    }
    try {
        const entries = fs.readdirSync(etapasDir, { withFileTypes: true });
...
export function cmdFindEtapa(cwd, etapa, raw) {
    if (!etapa) {
...
export function cmdEtapaPlanIndex(cwd, phase, raw) {
    if (!phase) {
    ...
    for (const planFile of planFiles) {
        ...
        if (fmFiles) {
            filesModified = Array.isArray(fmFiles) ? fmFiles : [String(fmFiles)];
        }
...
export function cmdEtapaAdd(cwd, description, raw) {
    if (!description) {
...
export function cmdEtapaInsert(cwd, afterPhase, description, raw) {
    ...
    try {
        ...
        for (const dir of dirs) {
            ...
            if (dm)
                existingDecimalNums.push(parseInt(dm[1], 10));
        }
    }
    catch { }
...
export function cmdEtapaRemove(cwd, targetPhase, options, raw) {
    if (!targetPhase) {
            ...
            for (const item of toRename) {
                ...
                for (const f of dirFiles) {
                    if (f.includes(oldPhaseId)) {
                        ...
                        fs.renameSync(path.join(etapasDir, newDirName, f), path.join(etapasDir, newDirName, newFileName));
...
export function cmdEtapaComplete(cwd, etapaNum, raw) {
    if (!etapaNum) {
    ...
    if (fs.existsSync(roadmapPath)) {
        let roadmapContent = fs.readFileSync(roadmapPath, 'utf-8');
        const checkboxPattern = new RegExp(`(-\\s*\\[)[ ](\\]\\s*.*Phase\\s+${escapeRegex(etapaNum)}[:\\s][^\\n]*)`, 'i');
        ...
        if (fs.existsSync(reqPath)) {
            const reqMatch = roadmapContent.match(new RegExp(`Phase\\s+${escapeRegex(etapaNum)}[\\s\\S]*?\\*\\*Requirements:\\*\\*\\s*([^\\n]+)`, 'i'));
            if (reqMatch) {
    ...
    if (fs.existsSync(statePath)) {
        let stateContent = fs.readFileSync(statePath, 'utf-8');
        stateContent = stateContent.replace(/(\*\*Current Phase:\*\*\s*).*/, `$1${nextPhaseNum || etapaNum}`);
        if (nextPhaseName) {
            stateContent = stateContent.replace(/(\*\*Current Etapa Name:\*\*\s*).*/, `$1${nextPhaseName.replace(/-/g, ' ')}`);
        }
        stateContent = stateContent.replace(/(\*\*Status:\*\*\s*).*/, `$1${isLastEtapa ? 'Marco completo' : 'Pronto para planejar'}`);
        stateContent = stateContent.replace(/(\*\*Current Plan:\*\*\s*).*/, `$1Não iniciado`);
        stateContent = stateContent.replace(/(\*\*Last Activity:\*\*\s*).*/, `$1${today}`);
        stateContent = stateContent.replace(/(\*\*Last Activity Description:\*\*\s*).*/, `$1Etapa ${etapaNum} complete${nextPhaseNum ? `, transitioned to Etapa ${nextPhaseNum}` : ''}`);
        writeStateMd(statePath, stateContent, cwd);
    }
    output({
