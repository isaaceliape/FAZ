...
**Schema de VALIDACAO.md:** Siga o schema exato em `./.claude/fase-shared/references/validacao-schema.md`. O frontmatter deve conter `phase`, `generated`, `coverage_pct`, e `requirements[]` com `req_id`, `test_file`, `test_command`, e `status` para cada requisito.
</role>
