...
node "./.claude/fase/bin/fase-tools.cjs" roteiro get-phase "$PHASE_NUM"
...
```bash
PHASE_DATA=$(node "./.claude/fase/bin/fase-tools.cjs" roteiro get-phase "$PHASE_NUM" --raw)
```
...
```bash
ARTIFACT_RESULT=$(node "./.claude/fase/bin/fase-tools.cjs" verify artifacts "$PLAN_PATH")
```
...
```bash
LINKS_RESULT=$(node "./.claude/fase/bin/fase-tools.cjs" verify key-links "$PLAN_PATH")
```
...
SUMMARY_FILES=$(node "./.claude/fase/bin/fase-tools.cjs" summary-extract "$PHASE_DIR"/*-SUMARIO.md --fields key-files)
...
if [ -n "$COMMIT_HASHES" ]; then
  COMMITS_VALID=$(node "./.claude/fase/bin/fase-tools.cjs" verify commits $COMMIT_HASHES)
fi
