...
```bash
INIT=$(node "$HOME/.fase-ai/bin/fase-tools.cjs" state load)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
```

Extraia `commit_docs` do init JSON. Resolva modelo do debugger:
```bash
debugger_model=$(node "$HOME/.fase-ai/bin/fase-tools.cjs" resolve-model faz-debugger --raw)
```
