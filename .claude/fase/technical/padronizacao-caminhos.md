...
- **Workflows**: `@./.claude/workflows/workflow-name.md`
- **Templates**: `@./.claude/templates/template-name.md`
- **Referências**: `@./.claude/references/reference-name.md`
...
```
Fonte:       @./.claude/workflows/plan-phase.md
...
```
Fonte:       ./.claude/workflows/add-phase.md
...
- `$HOME/.fase-ai/` → `<homePrefix>fase/` (por exemplo, `./.claude/fase/`)
...
- Têm referências inline `./.claude/` **preservadas** para o instalador substituir
...
Ao desenvolver localmente com arquivos de comando fonte do FASE (`comandos/*.md`), você precisa ter arquivos de workflow disponíveis em `./.claude/` para que o Claude Code carregue via blocos `<execution_context>`. Opções:
...
- <i class="fa fa-check-circle"></i> 13 comandos com referências de workflow usam `@./.claude/`
