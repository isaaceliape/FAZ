...
```bash
INIT=$(node "./.claude/fase/bin/fase-tools.cjs" state load)
...
```bash
node "./.claude/fase/bin/fase-tools.cjs" commit "docs: resolve debug {slug}" --files comandos/debug/resolved/{slug}.md
```
