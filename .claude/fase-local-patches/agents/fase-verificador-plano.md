...
```bash
INIT=$(node "./.claude/fase/bin/fase-tools.cjs" init phase-op "${PHASE_ARG}")
...
node "./.claude/fase/bin/fase-tools.cjs" roteiro get-phase "$phase_number"
...
for plan in "$PHASE_DIR"/*-PLANO.md; do
  echo "=== $plan ==="
  PLAN_STRUCTURE=$(node "./.claude/fase/bin/fase-tools.cjs" verify plan-structure "$plan")
  echo "$PLAN_STRUCTURE"
done
```
...
```bash
MUST_HAVES=$(node "./.claude/fase/bin/fase-tools.cjs" frontmatter get "$PLAN_PATH" --field must_haves)
```
...
```bash
PLAN_STRUCTURE=$(node "./.claude/fase/bin/fase-tools.cjs" verify plan-structure "$PLAN_PATH")
```
