...
// Timeout configurável para stdin (padrão: 5 segundos)
// Se stdin não fechar neste tempo, sai silenciosamente para evitar travamento
const STATUSLINE_TIMEOUT = parseInt(process.env.FASE_STATUSLINE_TIMEOUT || '5000', 10);

// Lê JSON do stdin
let input = '';
const stdinTimeout = setTimeout(() => {
  process.stderr.write(`[fase-statusline] Stdin timeout after ${STATUSLINE_TIMEOUT}ms\n`);
  process.exit(0);
}, STATUSLINE_TIMEOUT);
