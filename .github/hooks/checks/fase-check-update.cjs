...
// Timeout configurável para consulta npm (padrão: 10 segundos)
const NPM_TIMEOUT = parseInt(process.env.FASE_NPM_TIMEOUT || '10000', 10);
...
function validatePathInsideDir(dir, filePath) {
  const resolved = path.resolve(filePath);
  const normalizedDir = path.resolve(dir) + path.sep;
  return resolved === path.resolve(dir) || resolved.startsWith(normalizedDir);
}
...
// Valida que o arquivo de cache está dentro do diretório de cache
if (!validatePathInsideDir(cacheDir, cacheFile)) {
  console.error('FASE: Invalid cache file path detected');
  process.exit(1);
}
...
const child = spawn(process.execPath, ['-e', `
  ...
  try {
    ...
    console.error('FASE: Error reading version file:', e.message);
  }

  let latest = null;
  try {
    latest = execSync('npm view fase-ai version', { 
      encoding: 'utf8', 
      timeout: NPM_TIMEOUT, 
      windowsHide: true,
    ...
    console.error('FASE: npm check failed:', e.message);
  }
  ...
  try {
    ...
    console.error('FASE: Error writing cache:', e.message);
  }
`], {
  stdio: 'ignore',
  windowsHide: true,
