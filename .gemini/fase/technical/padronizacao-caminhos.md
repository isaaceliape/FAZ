...
- **Workflows**: `@./.gemini/workflows/workflow-name.md`
- **Templates**: `@./.gemini/templates/template-name.md`
- **Referências**: `@./.gemini/references/reference-name.md`
- **Ferramentas do runtime**: `$HOME/.fase-ai/bin/fase-tools.cjs`
...
- `~/\.fase/` → `<pathPrefix>fase/` (por exemplo, `./.gemini/fase/`)
- `$HOME/.fase-ai/` → `<homePrefix>fase/` (por exemplo, `./.gemini/fase/`)
...
- Têm referências inline `./.gemini/` **preservadas** para o instalador substituir
...
Esses arquivos são instalados na localização de caminho convertido (por exemplo, `./.gemini/fase/workflows/`, `~/.gemini/fase/workflows/`).

## Uso em Desenvolvimento

Ao desenvolver localmente com arquivos de comando fonte do FASE (`comandos/*.md`), você precisa ter arquivos de workflow disponíveis em `./.gemini/` para que o Claude Code carregue via blocos `<execution_context>`. Opções:

1. **Symlink para instalação do GSD:**
   ```bash
   ln -s ./.gemini/fase ~/.fase
   ```
...
- <i class="fa fa-check-circle"></i> 13 comandos com referências de workflow usam `@./.gemini/`
