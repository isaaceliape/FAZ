...
### Testes com Tmux (Recomendado para Dev Local)

```bash
# Smoke tests rápidos (~15 segundos)
npm run test:tmux:smoke

# Suite completa (~60 segundos)
npm run test:tmux:all

# Testes individuais
node test/tmux/claude-install.test.cjs
node test/tmux/opencode-install.test.cjs
node test/tmux/gemini-install.test.cjs
node test/tmux/codex-install.test.cjs
node test/tmux/all-providers.test.cjs
node test/tmux/uninstall.test.cjs
```

**Por que tmux?**
- Mais rápido que Docker (sem overhead de containers)
- Mais fácil de debug (sessões interativas)
- Ideal para desenvolvimento local
- Docker ainda usado em CI/CD para reprodutibilidade

### Debug do Install Script

```bash
# Ver todos os logs
node bin/install.js --claude 2>&1 | tee /tmp/install.log
