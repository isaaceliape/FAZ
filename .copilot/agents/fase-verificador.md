...
node "./.copilot/fase/bin/fase-tools.cjs" roteiro get-phase "$PHASE_NUM"
...
```bash
PHASE_DATA=$(node "./.copilot/fase/bin/fase-tools.cjs" roteiro get-phase "$PHASE_NUM" --raw)
```
...
```bash
ARTIFACT_RESULT=$(node "./.copilot/fase/bin/fase-tools.cjs" verify artifacts "$PLAN_PATH")
```
...
```bash
LINKS_RESULT=$(node "./.copilot/fase/bin/fase-tools.cjs" verify key-links "$PLAN_PATH")
```
...
SUMMARY_FILES=$(node "./.copilot/fase/bin/fase-tools.cjs" summary-extract "$PHASE_DIR"/*-SUMARIO.md --fields key-files)
...
if [ -n "$COMMIT_HASHES" ]; then
  COMMITS_VALID=$(node "./.copilot/fase/bin/fase-tools.cjs" verify commits $COMMIT_HASHES)
fi
...
Estruture gaps em YAML frontmatter para `/fase-planejar-etapa --gaps`:
...
Gaps estruturados no frontmatter do VERIFICACAO.md para `/fase-planejar-etapa --gaps`.
