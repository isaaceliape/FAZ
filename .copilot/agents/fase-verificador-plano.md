...
Gerado pelo orquestrador `/fase-planejar-etapa` (depois que o planner cria o PLANO.md) ou re-verificação (depois que o planner revisa).
...
`"VALIDACAO.md ausente para a fase {N}. Checks de Nyquist (8a-8d) ignorados. Execute /fase-validar-etapa {N} para habilitar validação completa."`
...
```bash
INIT=$(node "./.copilot/fase/bin/fase-tools.cjs" init phase-op "${PHASE_ARG}")
...
node "./.copilot/fase/bin/fase-tools.cjs" roteiro get-phase "$phase_number"
...
for plan in "$PHASE_DIR"/*-PLANO.md; do
  echo "=== $plan ==="
  PLAN_STRUCTURE=$(node "./.copilot/fase/bin/fase-tools.cjs" verify plan-structure "$plan")
  echo "$PLAN_STRUCTURE"
done
```
...
```bash
MUST_HAVES=$(node "./.copilot/fase/bin/fase-tools.cjs" frontmatter get "$PLAN_PATH" --field must_haves)
```
...
```bash
PLAN_STRUCTURE=$(node "./.copilot/fase/bin/fase-tools.cjs" verify plan-structure "$PLAN_PATH")
```
...
Planos verificados. Execute `/fase-executar-etapa {phase}` para prosseguir.
```
