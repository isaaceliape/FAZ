...
```bash
INIT=$(node "$HOME/.copilot-ai/bin/fase-tools.cjs" init execute-phase "${PHASE}")
...
**Validação de schema (schema completo em `./.copilot/fase-shared/references/plano-schema.md`):**
...
```bash
AUTO_CHAIN=$(node "$HOME/.copilot-ai/bin/fase-tools.cjs" config-get workflow._auto_chain_active 2>/dev/null || echo "false")
AUTO_CFG=$(node "$HOME/.copilot-ai/bin/fase-tools.cjs" config-get workflow.auto_advance 2>/dev/null || echo "false")
```
...
**Veja @./.copilot/references/checkpoints.md**
...
**Use template:** @./.copilot/templates/summary.md
...
node "$HOME/.copilot-ai/bin/fase-tools.cjs" state advance-plan

# Recalcula barra de progresso do estado em disco
node "$HOME/.copilot-ai/bin/fase-tools.cjs" state update-progress

# Registra métricas de execução
node "$HOME/.copilot-ai/bin/fase-tools.cjs" state record-metric \
  --phase "${PHASE}" --plan "${PLAN}" --duration "${DURATION}" \
  --tasks "${TASK_COUNT}" --files "${FILE_COUNT}"

# Adiciona decisões (extraia de key-decisions do SUMARIO.md)
for decision in "${DECISIONS[@]}"; do
  node "$HOME/.copilot-ai/bin/fase-tools.cjs" state add-decision \
    --phase "${PHASE}" --summary "${decision}"
done

# Atualiza info de sessão
node "$HOME/.copilot-ai/bin/fase-tools.cjs" state record-session \
  --stopped-at "Completed ${PHASE}-${PLAN}-PLANO.md"
```

```bash
# Atualiza progresso ROTEIRO.md para esta fase (contagem de plans, status)
node "$HOME/.copilot-ai/bin/fase-tools.cjs" roteiro update-plan-progress "${PHASE_NUMBER}"
...
node "$HOME/.copilot-ai/bin/fase-tools.cjs" requisitos mark-complete ${REQ_IDS}
```
...
```bash
node "$HOME/.copilot-ai/bin/fase-tools.cjs" state add-blocker "Descrição do bloqueador"
```
</state_updates>
...
$(node "$HOME/.copilot-ai/bin/fase-tools.cjs" summary-extract comandos/fases/${PHASE_DIR}/${PHASE}-${PLAN}-SUMARIO.md completed_tasks 2>/dev/null || echo "- Plano ${PHASE}-${PLAN} executado")

## Decisões Técnicas

$(node "$HOME/.copilot-ai/bin/fase-tools.cjs" summary-extract comandos/fases/${PHASE_DIR}/${PHASE}-${PLAN}-SUMARIO.md decisions 2>/dev/null || echo "- Ver SUMARIO.md")
...
$(node "$HOME/.copilot-ai/bin/fase-tools.cjs" state get-blockers 2>/dev/null || echo "- Nenhum")
...
<final_commit>
```bash
node "$HOME/.copilot-ai/bin/fase-tools.cjs" commit "docs({phase}-{plan}): complete [plan-name] plan" --files comandos/fases/XX-name/{phase}-{plan}-SUMARIO.md comandos/ESTADO.md comandos/ROTEIRO.md comandos/REQUISITOS.md
```
