...
```bash
node "./.copilot/fase/bin/fase-tools.cjs" websearch "sua query" --limit 10
```
...
```bash
INIT=$(node "./.copilot/fase/bin/fase-tools.cjs" init phase-op "${PHASE}")
...
```bash
node "./.copilot/fase/bin/fase-tools.cjs" commit "docs($PHASE): pesquisa domínio phase" --files "$PHASE_DIR/$PADDED_PHASE-PESQUISA.md"
```
