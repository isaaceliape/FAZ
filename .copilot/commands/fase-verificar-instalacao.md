...
test -f ./.copilot/settings.json && echo "EXISTS" || echo "NAO_EXISTE"
```
...
**Claude Code:**
```bash
ls ./.copilot/commands/fase-*.md 2>/dev/null | wc -l || echo "0"
```
...
```bash
ls ./.copilot/hooks/fase-*.js 2>/dev/null | wc -l || echo "0"
```

### 6. Verificar Arquivos de Workflows
```bash
test -d ~/.copilot && echo "EXISTS" || echo "NAO_EXISTE"
test -d ./.copilot/workflows && echo "EXISTS" || echo "NAO_EXISTE"
ls -la ./.copilot/workflows/*.md 2>/dev/null | wc -l || echo "0"
```
...
📁 WORKFLOWS FASE
  Diretório ~/.copilot: {EXISTS/MISSING}
...
```
✅ FASE. está instalado e configurado corretamente!
```
