...
- **Workflows**: `@./.copilot/workflows/workflow-name.md`
- **Templates**: `@./.copilot/templates/template-name.md`
- **Referências**: `@./.copilot/references/reference-name.md`
- **Ferramentas do runtime**: `$HOME/.copilot-ai/bin/fase-tools.cjs`
...
```
Fonte:       @./.copilot/workflows/plan-phase.md
Instalado:   @~/.gemini/fase/workflows/plan-phase.md
```

**Para Instalação no OpenCode:**
```
Fonte:       $HOME/.copilot-ai/bin/fase-tools.cjs
Instalado:   $HOME/.config/opencode/fase/bin/fase-tools.cjs
```

**Para Instalação no Claude Code:**
```
Fonte:       ./.copilot/workflows/add-phase.md
Instalado:   ./.copilot/fase/workflows/add-phase.md
```
...
- `~/\.fase/` → `<pathPrefix>fase/` (por exemplo, `./.copilot/fase/`)
- `$HOME/.copilot-ai/` → `<homePrefix>fase/` (por exemplo, `./.copilot/fase/`)

Para OpenCode especificamente:
- `~/.copilot` → `~/.config/opencode/fase`
- `$HOME/.copilot` → `$HOME/.config/opencode/fase`
...
- Têm referências inline `./.copilot/` **preservadas** para o instalador substituir
...
Esses arquivos são instalados na localização de caminho convertido (por exemplo, `./.copilot/fase/workflows/`, `~/.gemini/fase/workflows/`).

## Uso em Desenvolvimento

Ao desenvolver localmente com arquivos de comando fonte do FASE (`comandos/*.md`), você precisa ter arquivos de workflow disponíveis em `./.copilot/` para que o Claude Code carregue via blocos `<execution_context>`. Opções:

1. **Symlink para instalação do GSD:**
   ```bash
   ln -s ./.copilot/fase ~/.copilot
   ```
...
- <i class="fa fa-check-circle"></i> 13 comandos com referências de workflow usam `@./.copilot/`
