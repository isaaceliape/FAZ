...
```bash
ls .fase-ai-local/debug/*.md 2>/dev/null | grep -v resolved | head -5
```
</context>
...
<debug_file>
Criar: .fase-ai-local/debug/{slug}.md
</debug_file>
```
...
```bash
ls .fase-ai-local/debug/*.md 2>/dev/null | grep -v resolved | head -10
```
...
**Arquivo de debug:** .fase-ai-local/debug/{slug}.md
```
