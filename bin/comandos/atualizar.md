...
```bash
BACKUP_DIR=".fase-ai-local/.backup/pre-update-$(date +%Y%m%d-%H%M%S)"
mkdir -p "$BACKUP_DIR"
...
As migrações são rastreadas em `.fase-ai-local/migrations-applied`. Cada migração tem um
identificador único (M-NNN) e a versão em que foi introduzida.

```bash
MIGRATIONS_FILE=".fase-ai-local/migrations-applied"
touch "$MIGRATIONS_FILE" 2>/dev/null || mkdir -p .fase-ai-local && touch "$MIGRATIONS_FILE"

# Verificar quais migrações já foram aplicadas
migrations_aplicadas() { grep -q "$1" "$MIGRATIONS_FILE" 2>/dev/null; }
```

### M-001 — Renomear `.planejamento` → `.fase-ai-local` (introduzida em v2.x)

```bash
if ! migrations_aplicadas "M-001"; then
  if test -d .planejamento; then
    echo "M-001: Aplicando — renomear .planejamento → .fase-ai-local"
    ...
    grep -rl "\.planejamento" . \
      ...
      --exclude-dir=.gemini --exclude-dir=.codex | while read -r f; do
        sed -i '' 's/\.planejamento/.fase-ai-local/g' "$f"
    done

    # Atualizar .gitignore se necessário
    if ! grep -q "fase-ai-local" .gitignore 2>/dev/null; then
      sed -i '' 's/\.planejamento/.fase-ai-local/g' .gitignore 2>/dev/null || true
    fi
