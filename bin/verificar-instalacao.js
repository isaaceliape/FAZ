...
for (const runtime of runtimes) {
  ...
  if (isConfigured) {
    ...
    if (runtime.name === 'Codex') {
      hasSettings = fileExists(runtime.configFile);
      settingsLabel = 'Config';
    } else {
      hasSettings = fileExists(runtime.settingsFile);
    }
    ...
    if (runtime.name === 'Codex') {
      commandCount = countFilesRecursive(runtime.skillsDir, runtime.skillPattern);
      commandLabel = 'Skills FASE';
    } else if (runtime.commandsDir) {
