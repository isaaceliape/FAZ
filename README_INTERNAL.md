...
- Espaço em disco: Mínimo 1MB livre no diretório do projeto

### Variáveis de Ambiente (Opcional)

| Variável | Descrição | Padrão |
|----------|-----------|--------|
| `BRAVE_API_KEY` | Chave de API para Brave Search | Não definida (search desativado) |
| `CLAUDE_CONFIG_DIR` | Diretório customizado para Claude Code | `~/.claude` |
...
- `/fase-planejar-etapa` — Planeja uma fase
- `/fase-executar-etapa` — Executa fase
- `/fase-verificar-trabalho` — Verifica trabalho
- `/fase-progresso` — Mostra progresso
- `/fase-discutir-etapa` — Discute implementação
...
# ou
node dist/verificar-instalacao.js
```
...
Para documentação completa do FASE.:
