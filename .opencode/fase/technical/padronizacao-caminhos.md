...
- **Workflows**: `@./.opencode/workflows/workflow-name.md`
- **Templates**: `@./.opencode/templates/template-name.md`
- **Referências**: `@./.opencode/references/reference-name.md`
- **Ferramentas do runtime**: `$HOME/.config/opencode/fase-ai/bin/fase-tools.cjs`
...
```
Fonte:       @./.opencode/workflows/plan-phase.md
Instalado:   @~/.gemini/fase/workflows/plan-phase.md
```

**Para Instalação no OpenCode:**
```
Fonte:       $HOME/.config/opencode/fase-ai/bin/fase-tools.cjs
Instalado:   $HOME/.config/opencode/fase/bin/fase-tools.cjs
```

**Para Instalação no Claude Code:**
```
Fonte:       ./.opencode/workflows/add-phase.md
Instalado:   ./.opencode/fase/workflows/add-phase.md
```
...
- `~/\.fase/` → `<pathPrefix>fase/` (por exemplo, `./.opencode/fase/`)
- `$HOME/.config/opencode/fase-ai/` → `<homePrefix>fase/` (por exemplo, `./.opencode/fase/`)

Para OpenCode especificamente:
- `~/.config/opencode/fase` → `~/.config/opencode/fase`
- `$HOME/.config/opencode/fase` → `$HOME/.config/opencode/fase`
...
- Têm referências inline `./.opencode/` **preservadas** para o instalador substituir
...
Esses arquivos são instalados na localização de caminho convertido (por exemplo, `./.opencode/fase/workflows/`, `~/.gemini/fase/workflows/`).

## Uso em Desenvolvimento

Ao desenvolver localmente com arquivos de comando fonte do FASE (`comandos/*.md`), você precisa ter arquivos de workflow disponíveis em `./.opencode/` para que o Claude Code carregue via blocos `<execution_context>`. Opções:

1. **Symlink para instalação do GSD:**
   ```bash
   ln -s ./.opencode/fase ~/.config/opencode/fase
   ```
...
- <i class="fa fa-check-circle"></i> 13 comandos com referências de workflow usam `@./.opencode/`
