...
- Resultado é cacheado em `./.opencode/cache/fase-update-check.json`
- Se houver atualização, uma caixa estilizada é exibida
- Você é perguntado se deseja atualizar automaticamente

**Para verificar manualmente:**
```bash
node ./.opencode/fase-ai/fase-tools.js check-update $(cat ./.opencode/fase-ai/VERSION)
```

**Para desabilitar a verificação:**
Edite seu `./.opencode/settings.json` e remova o hook `fase-check-update.js` da seção `SessionStart`.
