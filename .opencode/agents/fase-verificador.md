...
node "./.opencode/fase/bin/fase-tools.cjs" roteiro get-phase "$PHASE_NUM"
...
```bash
PHASE_DATA=$(node "./.opencode/fase/bin/fase-tools.cjs" roteiro get-phase "$PHASE_NUM" --raw)
```
...
SUMMARY_FILES=$(node "./.opencode/fase/bin/fase-tools.cjs" summary-extract "$PHASE_DIR"/*-SUMARIO.md --fields key-files)
