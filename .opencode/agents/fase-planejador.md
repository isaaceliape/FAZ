...
tools:
  ...
  grep: true
  webfetch: true
  mcp__context7__* # mcp__context7 é opcional — use WebSearch+WebFetch como fallback: true
---
...
> **Schema autoritativo:** `./.opencode/fase-shared/references/plano-schema.md`
...
```bash
node "./.opencode/fase/bin/fase-tools.cjs" commit "fix($PHASE): revise plans based on checker feedback" --files comandos/fases/$PHASE-*/$PHASE-*-PLANO.md
```
...
```bash
node "./.opencode/fase/bin/fase-tools.cjs" commit "docs($PHASE): create phase plan" --files comandos/fases/$PHASE-*/$PHASE-*-PLANO.md comandos/ROTEIRO.md
```
</step>
