...
tools:
  ...
  websearch: true
  webfetch: true
  mcp__context7__* # mcp__context7 é opcional — veja fallback abaixo: true
---
...
Spawnado por `/fase-planejar-etapa` (integrado) ou `/fase-pesquisar-etapa` (standalone).
...
```bash
INIT=$(node "./.opencode/fase/bin/fase-tools.cjs" init phase-op "${PHASE}")
...
```bash
node "./.opencode/fase/bin/fase-tools.cjs" commit "docs($PHASE): pesquisa domínio phase" --files "$PHASE_DIR/$PADDED_PHASE-PESQUISA.md"
```
