...
test -f ./.opencode/settings.json && echo "EXISTS" || echo "NAO_EXISTE"
```
...
```bash
test -d ~/.config/opencode/fase && echo "EXISTS" || echo "NAO_EXISTE"
test -d ./.opencode/workflows && echo "EXISTS" || echo "NAO_EXISTE"
ls -la ./.opencode/workflows/*.md 2>/dev/null | wc -l || echo "0"
```
...
📁 WORKFLOWS FASE
  Diretório ~/.config/opencode/fase: {EXISTS/MISSING}
...
📁 WORKFLOWS FASE
  Diretório ~/.config/opencode/fase: EXISTS
