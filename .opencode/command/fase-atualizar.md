...
VERSAO_ATUAL=$(cat ./.opencode/fase-ai/VERSION 2>/dev/null || \
...
if ! migrations_aplicadas "M-002"; then
  OBSOLETOS=$(grep -l \
    "\.planejamento\|./.opencode/fase\|~/.config/opencode/fase\|~/.gemini/fase\|~/.codex/fase" \
...
```bash
grep -l "\.planejamento\|./.opencode/fase\|~/.config/opencode/fase\|~/.gemini/fase\|~/.codex/fase" \
  .claude/agents/fase-*.md .opencode/agents/fase-*.md \
  .gemini/agents/fase-*.md .codex/agents/*.toml 2>/dev/null
```
