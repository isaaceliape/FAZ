...
- **Workflows**: `@./.qwen/workflows/workflow-name.md`
- **Templates**: `@./.qwen/templates/template-name.md`
- **Referências**: `@./.qwen/references/reference-name.md`
...
```
Fonte:       @./.qwen/workflows/plan-phase.md
...
```
Fonte:       ./.qwen/workflows/add-phase.md
Instalado:   ./.qwen/fase/workflows/add-phase.md
```
...
- `~/\.fase/` → `<pathPrefix>fase/` (por exemplo, `./.qwen/fase/`)
- `$HOME/.fase-ai/` → `<homePrefix>fase/` (por exemplo, `./.qwen/fase/`)
...
- Têm referências inline `./.qwen/` **preservadas** para o instalador substituir
...
Esses arquivos são instalados na localização de caminho convertido (por exemplo, `./.qwen/fase/workflows/`, `~/.gemini/fase/workflows/`).

## Uso em Desenvolvimento

Ao desenvolver localmente com arquivos de comando fonte do FASE (`comandos/*.md`), você precisa ter arquivos de workflow disponíveis em `./.qwen/` para que o Claude Code carregue via blocos `<execution_context>`. Opções:
...
- <i class="fa fa-check-circle"></i> 13 comandos com referências de workflow usam `@./.qwen/`
