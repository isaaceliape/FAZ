...
- Resultado é cacheado em `./.qwen/cache/fase-update-check.json`
- Se houver atualização, uma caixa estilizada é exibida
- Você é perguntado se deseja atualizar automaticamente

**Para verificar manualmente:**
```bash
node ./.qwen/fase-ai/fase-tools.js check-update $(cat ./.qwen/fase-ai/VERSION)
```

**Para desabilitar a verificação:**
Edite seu `./.qwen/settings.json` e remova o hook `fase-check-update.js` da seção `SessionStart`.
