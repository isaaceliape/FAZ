...
> **Schema autoritativo:** `./.qwen/fase-shared/references/plano-schema.md`
...
```bash
node "./.qwen/fase/bin/fase-tools.cjs" commit "fix($PHASE): revise plans based on checker feedback" --files comandos/fases/$PHASE-*/$PHASE-*-PLANO.md
```
...
```bash
INIT=$(node "./.qwen/fase/bin/fase-tools.cjs" init plan-phase "${PHASE}")
...
```bash
node "./.qwen/fase/bin/fase-tools.cjs" history-digest
```
...
```bash
VALID=$(node "./.qwen/fase/bin/fase-tools.cjs" frontmatter validate "$PLAN_PATH" --schema plan)
```
...
```bash
STRUCTURE=$(node "./.qwen/fase/bin/fase-tools.cjs" verify plan-structure "$PLAN_PATH")
```
...
```bash
node "./.qwen/fase/bin/fase-tools.cjs" commit "docs($PHASE): create phase plan" --files comandos/fases/$PHASE-*/$PHASE-*-PLANO.md comandos/ROTEIRO.md
```
</step>
