...
test -f ./.qwen/settings.json && echo "EXISTS" || echo "NAO_EXISTE"
```
...
**Claude Code:**
```bash
ls ./.qwen/commands/fase-*.md 2>/dev/null | wc -l || echo "0"
```
...
```bash
ls ./.qwen/hooks/fase-*.js 2>/dev/null | wc -l || echo "0"
```

### 6. Verificar Arquivos de Workflows
```bash
test -d ~/.qwen && echo "EXISTS" || echo "NAO_EXISTE"
test -d ./.qwen/workflows && echo "EXISTS" || echo "NAO_EXISTE"
ls -la ./.qwen/workflows/*.md 2>/dev/null | wc -l || echo "0"
```
