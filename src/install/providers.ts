 ...
 * ```
 */
export function getConfigDirFromHome(runtime: ProviderRuntime, isGlobal: boolean): string {
  if (!isGlobal) {
 ...
 * ```
 */
export function getGlobalDir(runtime: ProviderRuntime, explicitDir: string | null = null): string {
  if (runtime === 'opencode') {
