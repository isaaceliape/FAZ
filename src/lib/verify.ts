...
export function cmdVerifyPhaseCompleteness(cwd: string, phase: string, raw: boolean): void {
  ...
  const planIds = new Set(plans.map((p: string) => p.replace(/-PLAN\.md$/i, '')));
  const summaryIds = new Set(summaries.map((s: string) => s.replace(/-SUMMARY\.md$/i, '')));

  const incompletePlans = [...planIds].filter((id: string) => !summaryIds.has(id));
  if (incompletePlans.length > 0) {
    errors.push(`Planos sem resumos: ${incompletePlans.join(', ')}`);
  }

  const orphanSummaries = [...summaryIds].filter((id: string) => !planIds.has(id));
...
export function cmdVerifyArtifacts(cwd: string, planFilePath: string, raw: boolean): void {
  ...
  const results: { path: string; exists: boolean; issues: string[]; passed: boolean }[] = [];
  for (const artifactRaw of artifacts) {
    if (typeof artifactRaw === 'string') continue;
    const artifact = artifactRaw as ArtifactItem;
    const artPath = artifact.path;
    if (!artPath) continue;

    const artFullPath = path.join(cwd, artPath);
    const exists = fs.existsSync(artFullPath);
    const check = { path: artPath, exists, issues: [] as string[], passed: false };
...
export function cmdValidateConsistency(cwd: string, raw: boolean): void {
  ...
  try {
    ...
    for (const dir of dirs) {
      ...
      const summaries = phaseFiles.filter((f: string) => f.endsWith('-SUMMARY.md'));
      const planIds = new Set(plans.map((p: string) => p.replace('-PLAN.md', '')));
      const summaryIds = new Set(summaries.map((s: string) => s.replace('-SUMMARY.md', '')));
  ...
  try {
    ...
    for (const dir of dirs) {
      const phaseFiles = fs.readdirSync(path.join(etapasDir, dir));
      const plans = phaseFiles.filter((f: string) => f.endsWith('-PLAN.md'));
