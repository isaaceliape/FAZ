...
export function cmdGenerateSlug(text: string, raw: boolean): void {
  if (!text) {
...
export function cmdCurrentTimestamp(format: string, raw: boolean): void {
  const now = new Date();
  let result: string;
...
export function cmdListTodos(cwd: string, area: string, raw: boolean): void {
  const pendingDir = path.join(cwd, '.fase-ai', 'todos', 'pending');

  let count = 0;
  const todos: { file: string; created: string; title: string; area: string; path: string }[] = [];
...
export function cmdVerifyPathExists(cwd: string, targetPath: string, raw: boolean): void {
  if (!targetPath) {
...
interface PhaseAccumSerialized {
  name: string;
  provides: string[];
  affects: string[];
  patterns: string[];
...
interface HistoryDigest {
  phases: Record<string, PhaseAccum | PhaseAccumSerialized>;
  decisions: { phase: string | number; decision: unknown }[];
...
export function cmdHistoryDigest(cwd: string, raw: boolean): void {
  ...
  const allPhaseDirs: { name: string; fullPath: string; milestone: string | null }[] = [];
  ...
  try {
    for (const { name: dir, fullPath: dirPath } of allPhaseDirs) {
      ...
      for (const summary of summaries) {
        try {
          ...
          const fm: ParsedFrontmatter = extractFrontmatter(content);

          const etapaNum = (fm['etapa'] as string | undefined) || dir.split('-')[0];
          ...
          const phase = (digest.phases as Record<string, PhaseAccum>)[etapaNum];
          ...
          if (depGraph?.provides && Array.isArray(depGraph.provides)) {
            (depGraph.provides as string[]).forEach((p: string) => phase.provides.add(p));
          } else if (fm['provides'] && Array.isArray(fm['provides'])) {
            (fm['provides'] as string[]).forEach((p: string) => phase.provides.add(p));
          }

          // Merge affects
          if (depGraph?.affects && Array.isArray(depGraph.affects)) {
            (depGraph.affects as string[]).forEach((a: string) => phase.affects.add(a));
          }

          // Merge patterns
          if (fm['patterns-established'] && Array.isArray(fm['patterns-established'])) {
            (fm['patterns-established'] as string[]).forEach((p: string) => phase.patterns.add(p));
          }

          // Merge decisions
          if (fm['key-decisions'] && Array.isArray(fm['key-decisions'])) {
            (fm['key-decisions'] as unknown[]).forEach((d: unknown) => {
              digest.decisions.push({ phase: etapaNum, decision: d });
            });
          }

          // Merge tech stack
          const techStack = fm['tech-stack'] as Record<string, unknown> | undefined;
          if (techStack?.added && Array.isArray(techStack.added)) {
            (techStack.added as (string | { name: string })[]).forEach((t) => {
    ...
    Object.keys(digest.phases).forEach((p: string) => {
      const phase = (digest.phases as Record<string, PhaseAccum>)[p];
      (digest.phases as Record<string, PhaseAccumSerialized>)[p] = {
        name: phase.name,
    ...
    error('Falha ao gerar histórico do projeto: ' + (e as Error).message);
  }
}

export function cmdResolveModel(cwd: string, agentType: string, raw: boolean): void {
  if (!agentType) {
...
interface SummaryExtractDecision {
  summary: string;
  rationale: string | null;
}

function parseDecisions(decisionsList: unknown): SummaryExtractDecision[] {
  if (!decisionsList || !Array.isArray(decisionsList)) return [];
  return (decisionsList as string[]).map((d: string) => {
...
export function cmdTodoComplete(cwd: string, filename: string, raw: boolean): void {
  if (!filename) {
  ...
  try {
    ...
    error(`Falha ao criar diretório completados: ${(err as Error).message}`);
  }
  ...
  try {
    ...
    error(`Falha ao ler tarefa: ${(err as Error).message}`);
  }
  ...
  try {
    ...
    try {
      ...
      process.stderr.write(`[cmdTodoComplete] Cleanup failed: ${(cleanupErr as Error).message}\n`);
    }
    error(`Falha ao completar tarefa: ${(err as Error).message}`);
  }
