...
export function cmdStateAdvancePlan(cwd: string, raw: boolean): void {
  ...
  if (isNaN(currentPlan) || isNaN(totalPlans)) {
    output(
      { error: 'Não é possível analisar Plano Atual ou Total de Planos da Fase do STATE.md' },
      raw
    );
    return;
  }
...
export function cmdStateRecordMetric(cwd: string, options: StateMetricOptions, raw: boolean): void {
  ...
  if (metricsMatch) {
    ...
    content = content.replace(
      metricsPattern,
      (_match, header: string) => `${header}${tableBody}\n`
    );
    writeStateMd(statePath, content, cwd);
    output({ recorded: true, phase, plan, duration }, raw, 'true');
  } else {
    output(
      { recorded: false, reason: 'Seção de Métricas de Desempenho não encontrada em STATE.md' },
      raw,
      'false'
    );
  }
...
export function cmdStateUpdateProgress(cwd: string, raw: boolean): void {
  ...
  if (boldProgressPattern.test(content)) {
    content = content.replace(
      boldProgressPattern,
      (_match, prefix: string) => `${prefix}${progressStr}`
    );
    writeStateMd(statePath, content, cwd);
    output(
    ...
    output(
      { updated: false, reason: 'Campo de Progresso não encontrado em STATE.md' },
      raw,
      'false'
    );
  }
...
export function cmdStateAddDecision(
  ...
  const sectionPattern =
    /(###?\s*(?:Decisões|Decisões Tomadas|Decisões Acumuladas)\s*\n)([\s\S]*?)(?=\n###?|\n##[^#]|$)/i;
...
export function cmdStateAddBlocker(
  cwd: string,
  text: string | StateBlockerOptions,
  raw: boolean
): void {
  const statePath = path.join(cwd, '.fase-ai', 'STATE.md');
  if (!fs.existsSync(statePath)) {
  ...
  const blockerOptions: StateBlockerOptions =
    typeof text === 'object' && text !== null ? text : { text: text as string };
  ...
  const sectionPattern =
    /(###?\s*(?:Bloqueadores|Bloqueadores\/Preocupações|Preocupações)\s*\n)([\s\S]*?)(?=\n###?|\n##[^#]|$)/i;
  const match = content.match(sectionPattern);
  ...
  if (match) {
    ...
    output(
      { added: false, reason: 'Seção de Bloqueadores não encontrada em STATE.md' },
      raw,
      'false'
    );
  }
...
export function cmdStateResolveBlocker(cwd: string, text: string | undefined, raw: boolean): void {
  ...
  if (match) {
    ...
    output(
      { resolved: false, reason: 'Seção de Bloqueadores não encontrada em STATE.md' },
      raw,
      'false'
    );
  }
...
export function cmdStateRecordSession(
  ...
  if (updated.length > 0) {
    ...
    output(
      { recorded: false, reason: 'Nenhum campo de sessão encontrado em STATE.md' },
      raw,
      'false'
    );
  }
...
export function cmdStateSnapshot(cwd: string, raw: boolean): void {
  ...
  const decisionsMatch = content.match(
    /##\s*(?:Decisões Tomadas|Decisions Made)[\s\S]*?\n\|[^\n]+\n\|[-|\s]+\n([\s\S]*?)(?=\n##|\n$|$)/i
  );
  if (decisionsMatch) {
  ...
  if (sessionMatch) {
    ...
    const lastDateMatch =
      s.match(/\*\*(?:Última Data|Last Date):\*\*\s*(.+)/i) ??
      s.match(/^(?:Última Data|Last Date):\s*(.+)/im);
    const stoppedAtMatch =
      s.match(/\*\*(?:Parado Em|Stopped At):\*\*\s*(.+)/i) ??
      s.match(/^(?:Parado Em|Stopped At):\s*(.+)/im);
    const resumeFileMatch =
      s.match(/\*\*(?:Arquivo para Retomar|Resume File):\*\*\s*(.+)/i) ??
